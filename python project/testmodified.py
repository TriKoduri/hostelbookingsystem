...
class signup_page(CenteredFrame):
    ...
    class RightFrame(tk.Frame):
        def __init__(self, master, *args, **kwargs):
            ...
            img = img.resize((550, 110), Image.LANCZOS)  # Resize the image if needed
        ...
        def validate_signup(self):
            ...
            if all((username, password, confirm_password)):
                ...
                if cursor.fetchone() is not None:
                    ...
                    cursor.execute('INSERT INTO users(usre_name, pasword, conform_pass) VALUES(?,?)',[username,password,confirm_password])
                    conn.commit()
                    conn.close()
...
class LoginPage(CenteredFrame):
        ...
        def create_widgets(self):
            ...
            username_label.place(relx=0.4, rely=0.4, anchor="e")  # Adjusted position
            password_label.place(relx=0.4, rely=0.55, anchor="e")
            self.username_entry.place(relx=0.5, rely=0.4, anchor="center")  # Adjusted position
...
class Hostel:
    def __init__(self, name, description, price, available_beds):
        self.name = name
...
hostels_data = [
    Hostel("Medha", "A cozy hostel with modern amenities.", "138000", 10),
    Hostel("Sarada", "Spacious rooms with beautiful views.", "80000", 15),
    Hostel("Vaishnavi", "Comfortable stay with recreational facilities.", "90000", 20),
    Hostel("Manasa", "Tranquil atmosphere and high-quality services.", "75000", 8),
    Hostel("Green Meadows", "Beautiful hostel surrounded by lush green meadows.", "100000", 12),
    Hostel("Srujana", "A hostel known for its serene environment ", "85000", 5),
    Hostel("Nirmala", "Modern hostel with state-of-the-art facilities.", "60000", 18),
    Hostel("Rohini", "Elegant hostel offering a peaceful stay ", "70000", 10)
]

class HostelSelectionPage(tk.Frame):
    def __init__(self, master=None):
        super().__init__(master)
        self.master = master
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        style = ttk.Style()
        style.configure("Treeview.Heading", font=("Helvetica", 12, "bold"))  # Make column headings bold
        style.configure("Treeview", font=("Helvetica", 12))  # Increase text size in treeview

        # Treeview widget for displaying hostels
        self.hostel_table = ttk.Treeview(table_frame, columns=("Name", "Description", "Price", "Available Beds"), show="headings", height=10)
        ...
        self.hostel_table.heading("Available Beds", text="Available Beds")

        # Set column widths
        self.hostel_table.column("Name", width=200)  # Adjust the width according to your preference
        self.hostel_table.column("Description", width=400)  # Adjust the width according to your preference
        self.hostel_table.column("Price", width=100)  # Adjust the width according to your preference
        self.hostel_table.column("Available Beds", width=150)  # Adjust the width according to your preference

        # Inserting data into the treeview
        for hostel in hostels_data:
            self.hostel_table.insert("", "end", values=(hostel.name, hostel.description, hostel.price, hostel.available_beds))

        # Packing the treeview widget
        self.hostel_table.pack(side="left", fill="both", expand=True)

        # Button to select hostel
    ...
    def select_hostel(self):
        ...
        if selected_item:
            ...
            selected_hostel_index = None
            for index, hostel in enumerate(hostels_data):
                if hostel.name == selected_hostel_name:
                    selected_hostel_index = index
                    break
            if selected_hostel_index is not None:
                selected_hostel = hostels_data[selected_hostel_index]
                if selected_hostel.available_beds > 0:
                    self.destroy()
                    self.master.hostel_page = HostelPage(self.master, selected_hostel_name)
                else:
                    messagebox.showwarning("Beds Not Available", "Sorry, all beds in this hostel have been booked.")
        else:
            messagebox.showwarning("Selection Error","Please select a hostel before submitting.")
class HostelPage(tk.Frame):
    ...
    def get_description(self, hostel_name):
        descriptions = {
            "Medha": "Medha Hostel offers a serene and welcoming environment conducive to both academic and personal growth. Situated amidst lush greenery, the hostel provides a tranquil atmosphere that promotes focus and relaxation. The architecture boasts modern design elements, ensuring residents' comfort and convenience.",
...
class RegistrationForm(CenteredFrame):
    def __init__(self, master=None, selected_hostel=None, **kwargs):
        ...
        resized_image = self.resize_image(image1, 740, 560)  # Resizing the image
        img_label = tk.Label(left_frame, image=resized_image, border=0, bg='white')
        img_label.image = resized_image  # Keep reference to prevent garbage collection
    ...
    def register(self):
        ...
        if not re.match(r"^[6-9]\d{9}$", phno):
            messagebox.showerror("Error", "Invalid phone number.")
            return

        if not re.match(r"^[6-9]\d{9}$", father_phno):
            messagebox.showerror("Error", "Invalid father's phone number.")
            return

        if not re.match(r"^[6-9]\d{9}$", mother_phno):
            messagebox.showerror("Error", "Invalid mother's phone number.")
            return
